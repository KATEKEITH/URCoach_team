...
class CoachUpdateForm extends Component {
    ...
    componentDidMount() {
        ...
        //
        fetch(`http://54.180.81.203/coaches/find/${memberId}`)
            .then(res => res.json()
            ).then(res => {
                this.setState({
                    coach: res,
    ...
    changePhoto = (e) => {
        ...
        fileReader.onload = function (e) {
            ...
            axios.post("http://54.180.81.203/upload", data, { headers: headers })
                .then(res => {
                    alert("사진이 업로드 되었습니다")
    ...
    handleChange = (e) => {
     ...
     updateGym = () => {
        ...
        console.log(data)
        axios.put(`http://54.180.81.203/gyms/update/${gymId}`, JSON.stringify(data), { headers: headers })
            .then( res => {
            alert("Gym 입력 성공")   
    ...
    updateCoach = () => {
      ...
      'Access-Control-Allow-Origin': '*'
        ...
        axios.put(`http://54.180.81.203/coaches/update/${this.state.coachId}`, JSON.stringify(data), { headers: headers })
            .then(res => {
                alert("수정성공")
            })
            .catch(e => { alert("작성실패") })

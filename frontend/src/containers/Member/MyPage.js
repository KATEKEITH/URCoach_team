...
class MyPage extends Component {
  ...
  componentDidMount () {
    ...
    axios
      .get(`http://54.180.81.203/members/find/${memberId}`)
      .then(res =>{
  ...
  changePhoto = (e) => {
    ...
    fileReader.onload = function(e){
      ...
      axios
        .post("http://54.180.81.203/upload",data,{headers: headers})
        .then(res => {
          alert("사진이 업로드 되었습니다.")
  ...
  updateMyPage = (e) => {
   ...
   //입력한 비밀번호가 null 값일 때 --> 기존의 입력 값 입력
    ...
    axios
      .put(`http://54.180.81.203/members/update/${memberId}`,JSON.stringify(data),{headers: headers})
      .then(res => {
        alert('수정 완료')
  ...
  deleteAccout = (e) => {
    ...
    let memberId = sessionStorage.getItem("memberId") 
    axios 
      .delete(`http://54.180.81.203/members/delete/${memberId}`)
      .then(res => {
        alert('삭제 성공')
  ...
  coachFormExists() {
    console.log('접근')
    let memberId = sessionStorage.getItem('memberId')
    axios.get(`http://54.180.81.203/coaches/exists/${memberId}`)
      .then(res => {
